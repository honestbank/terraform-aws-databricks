...
module "databricks_policy" {
  source     = "./modules/terraform-aws-iam/aws-iam/policy"
...
module "databricks_role" {
  source     = "./modules/terraform-aws-iam/aws-iam/role"
  aws_region = var.aws_region
  role = {
...
resource "aws_s3_bucket" "root_bucket" {
  ...
  bucket = "${local.prefix}-databricks-rootbucket-${random_string.naming.result}"
  acl    = "private"

  force_destroy = true

  tags = {
    Name = "${local.prefix}-databricks-rootbucket"
  }
  versioning {
    enabled = true
  }
